...
export async function getTotalRewardsEarned(
  ...
  const convertedStEth = await publicClient.readContract({
    ...
    const initialStakedAmount = firstDeposit.data ? BigInt(firstDeposit.data) : 0n;
    ...
    return toResult(`Total Rewards Earned: ${formatUnits(totalRewards, 18)} stETH`);
  } catch (error) {
    return toResult(`Failed to fetch total rewards: ${error instanceof Error ? error.message : "Unknown error"}`, true);
  }
}