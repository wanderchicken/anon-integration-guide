...
export async function getAverageAPR(
  ...
  try {
    ...
    const firstStakeTimestamp = await publicClient.getBlock({
      blockNumber: firstStakeBlock,
    });
    const currentTimestamp = Math.floor(Date.now() / 1000);
    const daysStaked =
      (currentTimestamp - Number(firstStakeTimestamp.timestamp)) / 86400;
    ...
    const apr = (
      (Number(rewardsEarned) / Number(initialStakedAmount)) *
      (365 / daysStaked) *
      100
    ).toFixed(2);

    return toResult(
      `Your Average APR over ${daysStaked.toFixed(2)} days: ${apr}%`
    );
  } catch (error) {
    return toResult(
      `Failed to fetch Average APR: ${
